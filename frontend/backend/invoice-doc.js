// ─────────────────────────────────────────────────────────────────────────────
// invoice-doc.js — SINGLE SOURCE OF TRUTH for the proforma invoice generator.
//
// Imported by BOTH backends:
//   • frontend/backend/index.js  → deployed on Render
//   • local-backend/index.js     → office Windows machine (requires this via
//                                   ../frontend/backend/invoice-doc.js)
//
// docx is injected by the caller (dependency injection) rather than required
// here, so this file has NO third-party dependencies of its own. That matters:
// the office machine only runs `npm install` inside local-backend/, so a
// require("docx") in this file would fail to resolve from frontend/backend/.
// By taking docx as a parameter, each backend supplies its own copy.
//
// Usage:
//   const { buildDoc, fmtAmt, sp, generateInvoiceBuffer } =
//     require("./invoice-doc")(require("docx"));
// ─────────────────────────────────────────────────────────────────────────────

module.exports = function createInvoiceDoc(docx) {
  const {
  ...
  // Build the .docx buffer plus the download filename headers. The caller is
  // responsible for request validation / field defaults and for sending the
  // HTTP response — this keeps the (identical) /generate route wiring in each
  // backend while the document + filename logic lives here, in one place.
  async function generateInvoiceBuffer(data) {
    ...
    return { buffer, filename, asciiName, contentDisposition };
  }

  return { fmtAmt, sp, buildDoc, generateInvoiceBuffer };
};
